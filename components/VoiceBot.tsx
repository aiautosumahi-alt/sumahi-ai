  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-2xl animate-fade-in">
          <div className="bg-slate-900 w-full max-w-xl rounded-[3rem] p-12 border border-white/10 shadow-2xl relative overflow-hidden flex flex-col items-center">
            ...
            <div className="mt-12 text-slate-600 text-[10px] font-bold tracking-tight uppercase opacity-50">
              Technical Encryption Active
            </div>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        @keyframes visualizer-low { 0%, 100% { height: 8px; opacity: 0.3; } 50% { height: 16px; opacity: 0.6; } }
        @keyframes visualizer-high { 0%, 100% { height: 12px; opacity: 0.6; } 50% { height: 40px; opacity: 1; } }
